    ...
    where T: DirFSInterface
{
    ...
    fn _readdir(
        ...
        let parent_attr = self.implementor.parent_attr();
        let dirs_offset = if parent_attr.is_some() { 2 } else { 1 };

        match parent_attr {
            Some(attr) if offset == 1 => {
...
impl<T> VirtualFS for DirFS<T>
    where T: DirFSInterface { }

impl<T> Filesystem for DirFS<T>
    where T: DirFSInterface
{
    fn lookup(&mut self, _req: &Request<'_>, parent: u64, name: &std::ffi::OsStr, reply: ReplyEntry) {
        if parent == self.implementor.inode() {
            if name == DIR_NAME_SELF {
                ...
                let Some(attr) = self.implementor.parent_attr()
...
impl<T> VirtualFile for DirFS<T>
